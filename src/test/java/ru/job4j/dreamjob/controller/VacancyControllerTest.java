...
public class VacancyControllerTest {
    ...
    @Test
    public void whenRequestVacancyPageThenGetPageWithCitiesAndVacancy() {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
    ...
    @Test
    public void whenSomeExceptionThrownUponVacancyIsEmptyThenGetErrorPageWithMessage() {
        ...
        var model = new ConcurrentModel();
        var view = vacancyController.getById(model, new Vacancy().getId());
        var actualExceptionMessage = model.getAttribute("message");
    ...
    @Test
    public void whenUpdateVacancyThenSameDataAndRedirectToVacanciesPage() {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
        when(vacancyService.update(vacancyArgumentCaptor.capture(),
                fileDtoArgumentCaptor.capture()))
                .thenReturn(vacancy.getVisible());
    ...
    @Test
    public void whenSomeExceptionThrownUponUpdateVacancyThenGetErrorPageWithMessage() {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        var expectedException = new RuntimeException("Vacancy with the specified ID was not found");
        ...
        var model = new ConcurrentModel();
        var view = vacancyController.update(
                new Vacancy(2, "test1", "desc1", now(), true, 1, 2), testFile, model);
        var actualExceptionMessage = model.getAttribute("message");
    ...
    @Test
    public void whenDeleteVacancyThenSameDataAndRedirectToVacanciesPage() {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
    ...
    @Test
    public void whenSomeExceptionThrownUponDeleteVacancyThenGetErrorPageWithMessage() {
        var expectedException = new RuntimeException("Vacancy with the specified ID was not found");
